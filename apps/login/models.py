...
class UserManager(models.Manager):
    def user_validation(self, postData):
        ...
        if len(postData['name']) < 3:
            errors.append("First name field cannot be blank and must be at least 3 characters in length.")
        ...
        if len(postData["hire"]) < 1:
            errors.append("Please provide your date of hire.")
        if postData["hire"] > str(date.today()):
			errors.append("That date is in the future and our time-machine is broken... Your hire by must be today or a past date.")
        ...
        if errors:
            ...
            user = self.create(name=postData['name'], user_name=postData['user_name'], password=postData['password'], hire=postData['hire'], salt=salt)
